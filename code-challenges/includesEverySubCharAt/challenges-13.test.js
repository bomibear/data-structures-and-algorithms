...
const firstLetters = (arr) => {
  ...
  //refactored:
  return arr.map(el => el.substr(0,1));
};
...
const findAnything = (arr, target) => {
  ...
  return arr.filter(containsTarget);
};
...
const findEvery = (arr, target) => {
  ...
  return arr.every(containsTarget);
};
...
const unenrollBrook = (arr) => {
  function containsStudent(students) {
    if(!students.includes('Brook')) return true;
  }
  let newArr = arr.map(el => el.filter(containsStudent));
  return newArr;
};
