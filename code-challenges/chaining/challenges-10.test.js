...
const count = (target, input) => {
  // org way:
  // let newArr = [].concat.apply([], input);
  // function equalsTarget(item) {
  //   if(item === target) return true;
  // }
  // return newArr.filter(equalsTarget).reduce(acc => acc + 1, 0);

  //refactored way
  return [].concat.apply([], input).filter(el => el === target ? true : false).reduce(el => el + 1, 0);
};
...
const totalSum = (input) => {
  return [].concat.apply([], input).reduce((acc, val) => acc + val);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  function filterArr(item) {
    if(typeof item[0] !== 'number') return [];
    else if(item[0] % 5 === 0) return item[0];
  }
  // let filterArr = input.filter(el => typeof el[0] !== 'number' || el[0] % 5 !== 0 ? false : true);
  console.log(input.filter(filterArr));
  return 0;
};
...
let findMaleAndFemale = (data) => {
  function checkGender(character) {
    if(character.gender === 'male' || character.gender === 'female') return true;
  }
  return data.filter(checkGender).map(obj => obj.name).join(' and ');
};
...
let findShortest = (data) => {
  ...
  let shortest = data.reduce((acc, curr) => {
    if(parseInt(acc.height) < parseInt(curr.height)) return acc;
    return curr;
  });
